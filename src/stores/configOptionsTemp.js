...
const limitFunction = {
    ...
    fontSize(style) {
        const fontPadding = (Number.parseFloat(style.lineHeight) - Number.parseFloat(style.fontSize)) / 2
        const bottomDistance = Number.parseFloat(style.height) - fontPadding - Number.parseFloat(style.fontSize)
        return { min: 12, max: fontPadding > bottomDistance ? (bottomDistance * 2 + Number.parseFloat(style.fontSize)) : (fontPadding * 2 + Number.parseFloat(style.fontSize)) }
    },
    lineHeight(style) {
        return { min: 20, max: Number.parseFloat(style.fontSize) + 2 * (Number.parseFloat(style.height) - Number.parseFloat(style.fontSize)) }
    },
